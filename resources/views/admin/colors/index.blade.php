...
@section('title')
    <span class="titlescc">@lang('trs.colors')</span>
@endsection

@section('content')
    <div class="card ticket">
        <h5 class="card-header">@lang('trs.colors_list')</h5>
                ...
                <table class="table txtcenter" style="width: 95%">
                    ...
                    @foreach($colors as $key=>$color)
                        <tr>
                            ...
                            <td data-th="عنوان">{{ $color->title }}</td>
                            <td data-th="توضیحات">{{ $color->description}}</td>
                            <td data-th="مدیریت">
                                <ul class="ulinlin fsize13">
                                    <li>
                                        <button style="background:none;border: none;"
                                                onclick='functionConfirm("آیا از حذف نوع وسیله نقلیه اطمینان دارید ؟", function yes() {
                                                    window.location.replace("{{route('admin.color.remove',$color->id)}}");
                                                    },
                                                    function no() {
                                                    });'>حذف
                                        </button>
                                    ...
                                    <li class="mgright10"><a class="no_hover_a"
                                                             href="{{route('admin.color.edit',$color->id)}}">ویرایش</a>
                                    </li>
                                </ul>
                                <div id="confirm">
